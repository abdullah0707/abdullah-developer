        ...
        <form class="space-y-10 text-sm mx-2" action="#" method="POST" @submit="SubmitForm(form)">
          ...
          <div class="user-box">
            <textarea id="message" name="message" v-model="form.message" type="textarea" required
              class="mt-1 py-3 px-2 border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-yellow-400 focus:ring-yellow-400 block w-full  sm:text-sm focus:ring-5" />
            <label for="message">
              <span class="after:content-['*'] after:ml-0.5 after:text-red-500 block text-sm font-bold  ">
                Message
              </span>
